...
dt_metadata:
  ...
  fecha: "2026-04-09"
  especialidad: "Eléctrica / Senalización"
...
Se identifica una distinción crítica entre las **obligaciones mandatorias (Ley del Contrato/Norma)** y las **propuestas de diseno (Criterios LFC)**. Este dictamen establece la hoja de ruta para evitar el sobredimensionamiento (Over-engineering) mediante la flexibilización de parámetros antes de su aprobación definitiva por la Interventoría.

### 1.1 Matriz de Trazabilidad: Autonomía y Carga

| Parámetro | Origen Normativo / Contractual | Criterio de Diseno (Propuesta LFC) | Carga de Ardanuy (Ingeniería Detalle) |
| :--- | :--- | :--- | :--- |
| **Autonomía 8h (Red)** | **Inexistente.** El AT3 §6.10 no cuantifica horas. | **4 Horas mínimas.** Propuesta ajustable basada en RAMS/Criticidad. | Dimensionamiento real + Factor Envejecimiento (1.25). |
| **Autonomía 48h (Solar)** | **Inexistente.** Sin mandato en Apéndices. | **44 Horas (2 días).** Estimación para radiación nula. | Dimensionamiento paneles + LiFePO4 según GHI local. |
...
1. **Flexibilización de Autonomía:** El valor de "4 horas" debe interpretarse como un rango propuesto (4h a 8h) dependiente de la criticidad del nodo. Se instruye a Ardanuy para no congelar este valor como un mínimo estricto en el DBCD sin un análisis de criticidad previo.
