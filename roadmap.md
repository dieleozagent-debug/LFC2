...
| D1.5 | **`II_A_Analisis_Contractual/dictamenes/` desalineado con BCD v001:** el `index.html` (558 líneas, "SICC Sovereign Registry — Reglas de Juego") + 20+ DTs muestran terminología pre-purga: "Red Vital IP", "Bus Vital 110V DC", "PTC Virtual (SICC) L2", "Soberano/Sovereign", "[REDACTADO_SICC]", "DT-SICC-V8-*". Producto del agente — depende de cerrar D1 antes. **Quitado del sidebar del index gerencial 2026-05-08** (commit pendiente) hasta que se sanee. Archivos siguen accesibles vía URL directa para equipo SICC interno. | (1) Cerrar D1 (sanear agente). (2) Regenerar dictamenes con BCD v001 vía `/promote`. (3) Reescribir `dictamenes/index.html` corporativo (sin "Reglas de Juego", "Sovereign", "Vital IP"). (4) Restaurar al sidebar del index. |
