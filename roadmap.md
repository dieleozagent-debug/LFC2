...
- [x] **P1-6 · URL canónica `/wbs`** — ✅ **CERRADO BIEN (`b927efc`)** tras el ciclo regresión→fix: el intento inicial (nav→`/wbs`, `98d3db6`) rompió la WBS Interactiva porque `/wbs` era un **rewrite** (sirve el HTML manteniendo la URL → assets relativos contra `/` → 404). Revertida la nav (`c24cb0f`) y luego los **8 alias de página** (`/wbs /vista /reporte /apartadero /ence /riesgos /entregables /master`) convertidos de **rewrite → redirect 301**: ahora la URL cambia al path real y los assets resuelven. Verificado en vivo: cada alias → final 200, 1 hop, sin loop. La nav queda en path directo (sin hop); `/wbs` es alias version-free funcional. **Lección:** cambios de routing se validan en deploy real (un rewrite no es lo mismo que un redirect para assets relativos).
...
- [x] **E6** — ✅ **Cerrado 2026-05-24** (`beac8ad`): hero de `WBS_Menu_Principal` ahora apunta a la WBS Interactiva real (antes decía "v4.0" pero el onclick iba a Vista_Final); Vista_Final reetiquetada como card secundaria. Borrado el "Brain Feed" con insights ficticios (+ CSS y setInterval). Pills/footer saneados ("v6.0/B.R.A.I.N./LFC Studio 2025" → "LFC UF2 · v14.7 · BCD v001"). Cifras vivas (`.js-item-count`), "25 riesgos"→48. Verificado en vivo.
- [x] **E8 — DTs visibles en modal "PROPONER DT"** — ✅ **Cerrado 2026-05-24** (`6d1ae02`): **root cause = caché (hipótesis c)**, no bug de lógica. La metadata/dataset/core se cargaban con `?v=14.7.5` (stale) mientras el resto iba en 14.7.14 → el browser servía metadata vieja sin `decisiones_tecnicas`. La lógica (`has()` + `.map()`) y el dato (array de strings) eran correctos (descartadas b y d). Fix: cache-bust unificado a `v=14.7.15`. Verificado: 24 ítems con DTs, `has()` truthy → bloque "DTs PREVIAS" renderiza. **Bonus aplicado:** badge "🛡️ N DT" en la vista de lista (tooltip con IDs, click abre el modal) para los 24 ítems.
...
| ~~D3~~ | ~~**Universo B (`datos_wbs_TODOS_items.js`)** con cifras pre-purga FENOCO~~. | ✅ **Cerrado 2026-05-24** (`4f9dfe1`): verificado que el ÚNICO consumidor que renderizaba cifras era `WBS_EDT_Detalle.html` (mostraba 1.1.103 = $63.112M pre-purga). Migrado a Universo A vía adaptador (`item→codigo`, `vu→vu_cop`, `total→total_cop`), verificado en vivo. Los 9 entregables solo tienen botón de descarga al `.json` (no renderizan) y el Audit Dashboard solo lo cita en un log → no mostraban cifras mal. El archivo Universo B queda en el repo (lo referencia el botón de descarga); su drop total es backlog. |
