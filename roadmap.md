...
| D1.5 | **`II_A_Analisis_Contractual/dictamenes/` vaciado el 2026-05-08** (commit pendiente): los 57 archivos eran producto v8 pre-purga del agente vía `/promote` durante pruebas tempranas (terminología "Red Vital IP", "Bus Vital 110V DC", "PTC Virtual (SICC) L2", "Soberano/Sovereign", "[REDACTADO_SICC]", "DT-SICC-V8-*", cifras "CAPEX Protegido $X M USD" sin trazabilidad). El directorio queda con un `README.md` placeholder explicando la regeneración esperada. El histórico forense se conserva en `agente/brain/dictamenes/` (no se toca). | Esperar cierre de D1 + D0 (Sección 10 YAML) en el agente, luego (a) `/audit [especialidad]` por especialidad con SSoT BCD v001, (b) `/promote` repuebla `LFC2/II_A_Analisis_Contractual/dictamenes/` con DTs alineados (sigla SCC en cara externa, no SICC), (c) reescribir `dictamenes/index.html` corporativo (sin "Reglas de Juego" / "Sovereign" / "Vital IP"), (d) restaurar al sidebar del index. |
