# WBS PRESUPUESTAL v4.0 - LÍNEA BASE MICHELIN
## APP La Dorada (México) - Chiriguaná - Presupuesto Maestro Saneado (SICC Michelin Phase)

**Fecha de actualización:** 09 de abril de 2026  
**Proyecto:** APP La Dorada - Chiriguaná  
**Contrato:** Concesión No. 001 de 2025  
**Versión:** 4.0 (Michelin Baseline)  
**Estado:** ✅ **PURGA FORENSE COMPLETADA - TRM 4400**

---

## 🛡️ REGISTRO DE SANEAMIENTO v4.0 (METODOLOGÍA P.42)
- ✅ **LÍNEA BASE MICHELIN:** Recalibración completa de CAPEX a TRM 4400.
- ✅ **PURGA FENOCO (SEC 25.4):** Eliminación de $77,112 millones COP de software Gateway [REDACTADO_SICC] (Obra Complementaria).
- ✅ **ALINEACIÓN TDR:** Presupuesto base para Términos de Referencia de licitación.

---

## 🏗️ ESTRUCTURA WBS PRESUPUESTAL v4.0

### CAPÍTULO 1: CONTROL Y SENALIZACIÓN VIRTUAL

#### 1.1 Sistema CTC Virtual Centralizado
| **Item** | **Descripción** | **Cantidad** | **VU (COP)** | **Total (COP)** |
|:---|:---|:---:|:---:|:---:|
| **1.1.100** | Servidores CTC principales | 2 | $180,000,000 | $360,000,000 |
| **1.1.101** | Servidores CTC respaldo | 2 | $180,000,000 | $360,000,000 |
| **1.1.102** | Workstations operadores | 8 | $45,000,000 | $360,000,000 |
| **1.1.103** | Software PTC Embarcado + Autorizaciones ($726M COP @ 4400) | 1 | $11,000,000,000 | $11,000,000,000 |
| **1.1.104** | Base de datos centralizada | 1 | $200,000,000 | $200,000,000 |
| **1.1.105** | Sistema redundancia | 1 | $300,000,000 | $300,000,000 |
| **1.1.106** | Interfaces comunicación | 1 | $150,000,000 | $150,000,000 |
| **SUBTOTAL CTC** | | | | **$12,430,000,000** |

#### **1.2 Sistema PTC Embarcado (15 Unidades — 1 U18 base AT1 §5 + 14 Factor Calidad — DT-CTRL-2026-006)**
| **Item** | **Descripción** | **Cantidad** | **VU (COP)** | **Total (COP)** |
|:---|:---|:---:|:---:|:---:|
| **1.2.100** | Computadora PTC embarcada | 15 | $120,000,000 | $1,800,000,000 |
| **1.2.101** | Display maquinista | 15 | $80,000,000 | $1,200,000,000 |
| **1.2.102** | Sensores velocidad | 15 | $25,000,000 | $375,000,000 |
| **1.2.103** | Sistema GPS | 15 | $35,000,000 | $525,000,000 |
| **1.2.104** | Software PTC | 15 | $100,000,000 | $1,500,000,000 |
| **1.2.105** | Interfaces frenos | 15 | $30,000,000 | $450,000,000 |
| **1.2.106** | Sistema redundancia | 15 | $50,000,000 | $750,000,000 |
| **SUBTOTAL PTC** | | | | **$6,600,000,000** |

#### **1.3 Enclavamientos ENCE (5 Estaciones)**
| **Item** | **Descripción** | **Cantidad** | **VU (COP)** | **Total (COP)** |
|:---|:---|:---:|:---:|:---:|
| **1.3.100** | Controlador ENCE Zapatosa | 1 | $800,000,000 | $800,000,000 |
| **1.3.101** | Controlador ENCE García Cadena | 1 | $800,000,000 | $800,000,000 |
| **1.3.102** | Controlador ENCE Barrancabermeja (Puerto) | 1 | $800,000,000 | $800,000,000 |
| **1.3.103** | Controlador ENCE Puerto Berrío-Grecia | 1 | $800,000,000 | $800,000,000 |
| **1.3.104** | Controlador ENCE La Dorada-México | 1 | $800,000,000 | $800,000,000 |
| **1.3.105** | Sistema redundancia ENCE | 1 | $400,000,000 | $400,000,000 |
| **1.3.106** | Interfaces CTC-ENCE | 5 | $60,000,000 | $300,000,000 |
| **SUBTOTAL ENCE** | | | | **$4,700,000,000** |

#### **1.4 Desvíos Motorizados (25 Unidades CTC/ENCE)**
| **Item** | **Descripción** | **Cantidad** | **VU (COP)** | **Total (COP)** |
|:---|:---|:---:|:---:|:---:|
| **1.4.100** | Desvíos motorizados CTC | 25 | $448,589,000 | $11,214,725,000 |
| **1.4.101** | Sistemas control remoto + redundancia N+1 | 25 | $134,577,000 | $3,364,425,000 |
| **1.4.102** | Interfaces CTC/ENCE | 25 | $67,288,000 | $1,682,200,000 |
| **SUBTOTAL DESVÍOS MOTORIZADOS** | | | | **$16,261,350,000** |

#### **1.5 Desvíos Manuales y Senalización**
| **Item** | **Descripción** | **Cantidad** | **VU (COP)** | **Total (COP)** |
|:---|:---|:---:|:---:|:---:|
| **1.5.100** | Desvíos manuales autotalonables (con comprobador integrado) | 120 | $134,577,000 | $16,149,240,000 |
| **1.5.101** | Senalización luminosa estaciones ENCE (entrada/salida) | 0 | $0 | $0 |
| **1.5.102** | Herramientas operación (kit completo certificado) | 120 | $13,458,000 | $1,614,960,000 |
| **SUBTOTAL DESVÍOS MANUALES Y SENALIZACIÓN** | | | | **$17,764,200,000** |

**TOTAL CAPÍTULO 1:** **$57,755,550,000**

---

### **CAPÍTULO 2: TELECOMUNICACIONES COLOCALIZADAS**

#### **2.1 Sistema TETRA (37 Estaciones)**
| **Item** | **Descripción** | **Cantidad** | **VU (COP)** | **Total (COP)** |
|:---|:---|:---:|:---:|:---:|
| **2.1.100** | Torres autosoportadas | 37 | $180,000,000 | $6,660,000,000 |
| **2.1.115** | Repuestos (10%) | - | - | $1,774,000,000 |
| **SUBTOTAL TETRA** | | | | **$19,038,000,000** |

#### **2.2 Sistema Red Vital IP / TETRA (37 Estaciones Colocalizadas)**
| **Item** | **Descripción** | **Cantidad** | **VU (COP)** | **Total (COP)** |
|:---|:---|:---:|:---:|:---:|
| **SUBTOTAL Red Vital IP / TETRA** | | | | **$3,618,000,000** |

#### **2.3 Fibra Óptica (526 km)**
| **Item** | **Descripción** | **Cantidad** | **VU (COP)** | **Total (COP)** |
|:---|:---|:---:|:---:|:---:|
| **2.3.100** | Cable fibra óptica monomodo G.652.D **48 hilos** (BCD §6.1.1) | 579 km | $9,811,013 | $5,680,576,527 |
| **2.3.101** | Mano de obra instalación (Obra Civil) | 579 km | $90,188,987 | $52,219,423,473 |
| **SUBTOTAL FIBRA ÓPTICA** | | | | **$61,282,093,560** |

**TOTAL CAPÍTULO 2:** **$92,672,367,851**

---

### **💰 RESUMEN PRESUPUESTAL TOTAL — POST-AUDITORÍA 2026-05-04**

> ⚠️ **REVISIÓN POST-AUDIT (2026-05-04):** Este resumen refleja los totales sincronizados con `wbs_presupuestal_datos.js` tras la auditoría doctrinal SICC v14.7. Los totales por capítulo originales del Michelin Baseline (09-04-2026) están preservados al final de este documento como **"Anexo Histórico v4.0"**. Cambios aplicados:
>
> - **+$1,557.6M (Cap 1):** Restituido ítem 1.5.101 (Señalización semafórica estaciones ENCE — gap doctrinal AT1 Tabla 17 + BCD §Tabla 2).
> - **+$120M (Cap 2):** OTDR 2.3.106 upgrade a triwavelength (1310/1550/1625 nm).
> - **+$1,850M (Cap 2):** Nuevo ítem 2.1.116 — bancos baterías LiFePO4 24h sitios remotos TETRA.
> - **+$1,200M (Cap 2):** Nuevo subcap 2.4.100 — Habilitación Satelital Transitoria (Punto 42, DT-SICC-2026-042, cadena 9.12 → 25.4).
> - **Cap 5 reconciliado:** $7,600M (Michelin .md original) → $16,412M (.js operativo). El .md histórico no incluía SCADA software ($5,000M) ni interfaces comunicación ($4,112M) que sí están en el .js. **Línea válida: la del .js.**

| **Capítulo** | **Descripción** | **Total (COP) Post-Audit** |
|:---|:---|:---:|
| **1** | Control y Senalización Virtual | $59,613,150,000 |
| **2** | Telecomunicaciones Colocalizadas | $88,113,093,560 |
| **3** | Sistemas ITS y Seguridad | $5,382,300,000 |
| **4** | Pasos a Nivel | $18,238,000,000 |
| **5** | Centro de Control Operacional | $16,412,000,000 |
| **6** | Material Rodante | $12,647,500,000 |
| **SUBTOTAL (COSTO DIRECTO)** | | **$200,406,043,560** |

### **📈 ESTRUCTURA FINANCIERA (TRM 4400)**

#### **CÁLCULO AIU (33% SOBRE OBRA CIVIL):**
| **Concepto** | **Valor (COP)** |
|:---|:---:|
| **Base Obra Civil (Fiber + Civil Items)** | $59,367,258,278 |
| **AIU TOTAL (33%)** | **$19,591,195,232** |

#### **CÁLCULO IVA:**
| **Concepto** | **Base (COP)** | **Valor (COP)** |
|:---|:---:|:---:|
| **IVA Suministros/Servicios (19%)** | $136,285,859,573 | $25,894,313,319 |
| **IVA Sobre Utilidad (19% of 5% Utility)** | $2,968,362,914 | $563,988,954 |
| **IVA TOTAL** | | **$26,458,302,273** |

### **🏁 PRESUPUESTO TOTAL CONSOLIDADO POST-AUDIT (2026-05-04):**
| **Concepto** | **Valor (COP)** | **Valor (USD @ 4400)** |
|:---|:---:|:---:|
| **Costo Directo** | $200,406,043,560 | $45,546,828 |
| **AIU TOTAL (calc. en vivo desde wbs_core_logic.js)** | (recalcular en `WBS_Vista_Final.html`) | — |
| **IVA TOTAL (calc. en vivo)** | (recalcular en `WBS_Vista_Final.html`) | — |
| **TOTAL ESTIMADO POST-AUDIT** | **~$247,250M (estimado +AIU+IVA)** | **~$56.2M** |

> **Fuente de verdad operativa:** `IX_WBS_Planificacion/WBS_Vista_Final.html` calcula en vivo desde `wbs_presupuestal_datos.js` (135 ítems). Los valores AIU/IVA exactos se obtienen ahí.

---

### **📜 ANEXO HISTÓRICO — v4.0 Michelin Baseline (09-04-2026)**

Totales originales pre-auditoría 2026-05-04, conservados como referencia histórica:

| **Capítulo** | **Total (COP) Michelin Original** |
|:---|:---:|
| 1. Control y Senalización Virtual | $57,755,550,000 |
| 2. Telecomunicaciones Colocalizadas | $92,672,367,851 |
| 3. Sistemas ITS y Seguridad | $3,395,700,000 |
| 4. Pasos a Nivel | $21,582,000,000 |
| 5. Centro de Control Operacional | $7,600,000,000 |
| 6. Material Rodante | $12,647,500,000 |
| **SUBTOTAL Original** | **$195,653,117,851** |
| Costo Directo | $195,653,117,851 / $44,466,618 USD |
| AIU TOTAL | $19,591,195,232 / $4,452,544 USD |
| IVA TOTAL | $26,458,302,273 / $6,013,251 USD |
| **TOTAL LÍNEA BASE ORIGINAL** | **$241,702,615,356 / $54,932,413 USD** |

---
**Control de Versiones:**
- v4.0 Michelin Baseline — 09-04-2026 (anexo histórico arriba)
- v4.0-post-audit — 2026-05-04 (totales vigentes; sincronizado con .js)

**Firmado:** Dirección Técnica UF2 — LFC
